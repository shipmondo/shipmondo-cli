...
app = typer.Typer(help="Manage sales_order_packagings")

@app.command("list")
def list_cmd(
    param_id: int = typer.Option(None, "--id", help="ID for the sales order packaging to be included in the filter"),
    ...
    if help_json:
        print("""{
  "command": "shipmondo sales_order_packagings list",
  ...
  "payload_schema": null
    ...
    endpoint = f"/sales_order_packagings"
    url_params = {}
...
@app.command("get")
def get_cmd(
    param_id: int = typer.Argument(None, help="ID for the sales order packaging to be included in the filter"),
    query: str = typer.Option(None, "--query", help="JSON string fallback for raw custom GET parameters"),
    json_output: bool = typer.Option(True, "--json/--text", help="Force JSON output"),
    debug: bool = typer.Option(False, "--debug", help="Print raw API requests and responses to stderr"),
    open_pdf: bool = typer.Option(False, "--open-pdf", help="Find, decode, and open base64 PDFs in response"),
    ...
    if help_json:
        print("""{
  "command": "shipmondo sales_order_packagings get",
  ...
  "payload_schema": null
    ...
    client = ShipmondoClient(debug=debug)
    endpoint = f"/sales_order_packagings/{param_id}"
    url_params = {}
    if query:
        url_params.update(json.loads(query))
    data = client.request("GET", endpoint, params=url_params)
